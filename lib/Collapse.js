...
var _initialiseProps = function _initialiseProps() {
  ...
  this.getWrapperStyle = function (height) {
    if (_this2.state.currentState === IDLING && _this2.state.to) {
      ...
      if (fixedHeight > -1) {
        return { overflow: 'hidden', visibility: _this2.getVisibility(fixedHeight), height: fixedHeight };
      }
    ...
    return { overflow: 'hidden', visibility: _this2.getVisibility(height), height: Math.max(0, height) };
  };

  this.getVisibility = function (height) {
    if (height <= 0) {
      return 'hidden';
    }
    return 'initial';
